...
parser.add_argument(
    "--working_dir", default="working_dir/", help="dir to sync with fswatch"
)
...
if __name__ == "__main__":
    ...
    if args.skip_ssh_key:
        ...
        try:
            ...
            output = subprocess.run(
                f"gcloud compute tpus tpu-vm scp ~/.ssh/pod_key.pub {args.tpu_name}:.ssh/pod_key.pub --worker=all --zone={args.zone}",
                shell=True,
                check=True,
            )
            # give all workers the private key - we could only give worker 0: TODO: Check security implications
            output = subprocess.run(
                f"gcloud compute tpus tpu-vm scp ~/.ssh/pod_key {args.tpu_name}:.ssh/pod_key --worker=all --zone={args.zone}",
                shell=True,
                check=True,
            )
            ...
            print(
                "Note - if this threw an error, Follow the fix provided in the gcp output, then rerun this script with --skip_ssh_connection."
            )
    ...
    if args.skip_ssh_connection:
        print(
            "Skipping connecting to hosts - you indicated you already have separate tmux windows with each host connected."
        )
    else:
